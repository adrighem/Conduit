...
Add GNOME-native Slack huddle support to Conduit: discover active huddles from Slack's supported metadata and events, present trustworthy join and in-call controls, provide a native GStreamer/PipeWire WebRTC media engine and synthetic session harness for audio, camera, and portal-based screen sharing, and isolate Slack's undocumented first-party bootstrap plus Amazon Chime signalling behind capability-checked adapters. When a verified Slack/Chime bridge is unavailable or changes, keep discovery useful and offer an explicit external Slack fallback instead of failing silently or misusing the Calls API.
...
5. Implement a native media controller around GStreamer and `webrtcbin`, with Opus audio, camera/video tracks, generic ICE/SDP negotiation for the synthetic harness, bounded latency, explicit teardown, media-device discovery, and redacted statistics. This engine must not claim standalone compatibility with Amazon Chime.
...
7. Keep Slack-specific `rooms.join` bootstrap and Amazon Chime signalling/media bridging behind narrow, independent adapters. Undocumented browser-session behavior must be capability checked, feature isolated, comprehensively redacted, and replaceable without changing the coordinator or generic media engine. Native Slack joining remains unavailable unless both adapters report a verified compatible capability.
...
- A synthetic end-to-end session exercises generic SDP/ICE exchange, audio/video graph construction, mute/camera changes, reconnect, statistics, and immediate leave teardown without representing itself as a Slack/Chime interoperability test.
...
- Native Slack join is attempted only when both the private Slack bootstrap adapter and Chime bridge report verified compatible capabilities. Otherwise the user receives a clear external-open fallback.
...
- The Debian 13 baseline uses the portal's restricted PipeWire file descriptor and selected node ID. Stream serial targeting may replace the node ID only when the deployed portal and compatible Rust bindings expose the version 6 contract.
...
- Claiming generic `webrtcbin` offer/answer signalling can join an Amazon Chime meeting without a verified Chime bridge.
- Bundling Chromium, Firefox, Google libwebrtc, or Amazon's C++ signalling SDK before a maintainable packaged integration is proven.
